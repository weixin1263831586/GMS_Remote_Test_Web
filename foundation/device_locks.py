...
Cross-cutting infrastructure for devices, cluster, system and workflows.
Owner display names come from platform identity, which foundation cannot
import; the resolver is injected via :func:`configure_display_name_resolver`.
"""
...
DisplayNameResolver = Callable[[str, str | None], str]
_display_name_resolver: DisplayNameResolver | None = None


def configure_display_name_resolver(resolver: DisplayNameResolver | None) -> None:
    """Register (or clear) the owner display-name resolver."""
    global _display_name_resolver
    _display_name_resolver = resolver


def _default_display_username(client_id: str, username: str | None) -> str:
...
class DeviceLockManager:
    ...
    def _display_username(self, client_id: str, username: str | None) -> str:
        cleaned = str(username or "").strip()
        if _display_name_resolver is not None:
            with contextlib.suppress(Exception):
                return _display_name_resolver(client_id, cleaned)
        return _default_display_username(client_id, username)
