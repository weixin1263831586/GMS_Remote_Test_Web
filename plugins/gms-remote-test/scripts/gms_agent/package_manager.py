...
def sandbox_home_reason() -> str:
    """Return a human reason when Path.home() is an ephemeral location.

    Installers executed inside an AI-client/tool sandbox frequently get a
    redirected ``$HOME`` (e.g. ``/tmp/tmpXXXXXXXX``). Everything the install
    writes — runtime versions, ``bin`` links, profiles — then lands in that
    temp directory and dies with it, producing the recurring
    "gms-rt links are dead" failure. Fail closed instead.
    """
    home = Path.home().resolve()
    for parent in ("/tmp", "/var/tmp", "/dev/shm"):
        marker = Path(parent)
        if home == marker or marker in home.parents:
            return (
                f"HOME resolves to ephemeral path {home} (under {parent}); "
                "installing here would vanish with the sandbox. Re-run the "
                "installer from a real login shell (or unset the sandbox "
                "HOME), or set GMS_INSTALL_ALLOW_SANDBOX_HOME=1 to override."
            )
    return ""


def _is_ephemeral_path(path: Path) -> bool:
    """Whether the path lives under a location cleaned with the OS tmp."""
    try:
...
def _resolve_runtime_target() -> Path:
    try:
...
def install_cli_dispatcher() -> list[Path]:
    ...
    The dispatcher resolves the runtime root at INVOCATION time from the
    environment (never embeds the install-time absolute path): embedding
    ``$HOME/...`` broke every link when the binary was later used from a
    different login context than the installer ran in.
    """
    ...
    # 二次防线：只有「持久 bin 目录 + 临时 runtime」这个危险组合才拒绝。
    # 插件更新器在沙箱里装 runtime、却把链接写进真实 ~/.local/bin 时，
    # /tmp 一清 gms-agent 即成死链（16:29:51 事故）。bin 与 runtime 同在
    # 同一临时根（自洽的测试/CI 场景）不受影响。
    if not _is_ephemeral_path(bin_dir):
        runtime_target = _resolve_runtime_target()
        if _is_ephemeral_path(runtime_target) and (
            os.environ.get("GMS_INSTALL_ALLOW_SANDBOX_HOME") != "1"
        ):
            raise SystemExit(
                f"Error: runtime root {runtime_target} is ephemeral but bin "
                f"dir {bin_dir} is persistent; linking would create dead "
                "commands once the temp dir is cleaned. Re-run the install "
                "from a real login shell, or set "
                "GMS_INSTALL_ALLOW_SANDBOX_HOME=1 to override."
            )
    ...
    dispatcher.write_text(
        ...
        "# Resolves the runtime at invocation time — never embed the\n"
        "# install-time absolute path (it dies when HOME/sandbox changes).\n"
        'invoked="${0##*/}"\n'
        'runtime_root="${GMS_AGENT_RUNTIME_ROOT:-'
        '${XDG_DATA_HOME:-$HOME/.local/share}/gms-remote-test}"\n'
        'cli="$runtime_root/current/scripts/gms-remote-test.sh"\n'
        'if [ "$invoked" = "gms-rt" ] && [ "$#" -gt 0 ] && [[ "$1" == gms-rt-* ]]; then\n'
        '    exec bash "$cli" "$@"\n'
        "fi\n"
        'if [[ "$invoked" == gms-rt-* ]]; then\n'
        '    exec bash "$cli" "$invoked" "$@"\n'
        "fi\n"
        '    exec bash "$cli" "$@"\n',
        encoding="utf-8",
    )
    ...
    for candidate in bin_dir.glob("gms-rt-*"):
        if candidate.name in links or not candidate.is_symlink():
            continue
        # 悬空链接（指向已消失的 runtime/沙箱 HOME）直接摘除：留着只会
        # 在补全里冒出假命令，用户一敲就是 exec 失败。
        if not candidate.exists() or Path(os.readlink(candidate)) == dispatcher:
            candidate.unlink()
