...
2026-09-11 反馈（反馈 2026-09-11）：status 需要直接暴露 principal_type /
...
class AuthStatusFieldTests(AuthApiTests):
    def test_auth_status_reports_principal_type_and_credential_mode(self):
        """2026-09-11 反馈回归：status 需要直接暴露 principal_type/credential_mode。"""
        self.client.post(
            "/api/auth/setup",
        ...
        # 2026-09-11 反馈：authenticated=true 时不应再要求出示凭据；needs_authentication
