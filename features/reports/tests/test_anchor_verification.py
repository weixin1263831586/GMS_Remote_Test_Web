"""Wiki→codesearch 串联验证管线测试（ADR 0014 阶段 ②-④）。

verify_background_anchors 把 background 命中的 source_anchors 逐一送到
本地 codesearch 验证文件实体；这里用桩替换 run_codesearch_process，
不真正起子进程。
"""
...
class _CodesearchStub:
    ...
    def __call__(self, cmd: list[str], cwd: str):
...
class AnchorVerificationTests(unittest.TestCase):
    def test_verified_anchor_reports_local_path(self):
    ...
    def test_unverified_anchor_is_false_not_removed(self):
    ...
    def test_url_only_anchor_is_skipped_without_search(self):
        ...
        # url-only anchor 无从验证：整体跳过，不产生条目。
    ...
    def test_budget_caps_codesearch_calls_and_dedups(self):
        ...
        self.assertEqual(len(out), av.MAX_VERIFIED_ANCHORS)

    def test_crashing_verifier_degrades_to_unknown(self):
        def boom(cmd: list[str], cwd: str):
        ...
        self.assertEqual(len(out), 1)
        self.assertIsNone(out[0]["verified"])
    ...
    def test_same_basename_at_wrong_path_is_not_verified(self):
