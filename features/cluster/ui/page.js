...
function render(){
 ...
 document.querySelector('#workers').innerHTML=state.workers.map(worker=>{
  ...
  const configButton=`<button class="worker-config" data-worker-id="${esc(worker.id)}" onclick="openWorkerConfig(this.dataset.workerId)" title="配置参数">⚙ 配置</button>`;
  const menuItems=[];
  if(worker.capabilities?.ssh_user)menuItems.push(`<button onclick="restartWorkerVnc('${esc(worker.id)}')">重启桌面</button>`);
  if(worker.id!==localId)menuItems.push(`<button onclick="redeployWorker('${esc(worker.id)}','${esc(sshHost)}')">重新部署</button>`);
  if(worker.id!==localId)menuItems.push(`<button class="danger" onclick="deleteWorker('${esc(worker.id)}')" ${deleteBlocked?'disabled':''}>${deleteBlocked?'删除(请先停测试)':'删除主机'}</button>`);
  const moreMenu=menuItems.length?`<span class="worker-menu-wrap"><button class="worker-menu-toggle" onclick="event.stopPropagation();this.nextElementSibling.classList.toggle('open')">⋯</button><div class="worker-menu" onclick="event.stopPropagation()">${menuItems.join('')}</div></span>`:'';
  return `<div class="card"><div class="card-title"><span>${esc(worker.name||worker.id)}</span><span>${badge(worker.status)}${configButton}${moreMenu}</span></div><p>${esc(worker.hostname)} · ${esc(worker.id)} · ${esc(worker.address||'')}</p><div class="meta"><div class="meta-row"><span>CPU ${oneDecimal(worker.cpu_percent)}%</span><span>内存 ${oneDecimal(worker.memory_percent)}%（可用 ${oneDecimal(worker.memory_available_gb)}G）</span><span>磁盘 ${oneDecimal(worker.disk_free_gb)}G</span><span>系统负载 ${oneDecimal(worker.load_1m)}</span></div><div class="meta-row"><span>设备 ${assessment.devices.length}</span><span>套件 ${state.suites.filter(suite=>suite.worker_id===worker.id&&suite.available).length}</span><span>任务 ${worker.running_jobs}/${worker.max_jobs}（外部 ${worker.external_jobs||0}）</span></div></div><div class="host-assessment"><span class="assessment ${assessment.loadClass}">负载：${assessment.loadLabel}</span></div><div class="capabilities"><span class="cap ${worker.capabilities?.ssh_user?'ok':''}">终端 ${worker.capabilities?.ssh_user?'✓':'未配置'}</span><span class="cap ${worker.capabilities?.novnc_port?'ok':''}">noVNC ${worker.capabilities?.novnc_port?'✓':'不可用'}</span><span class="cap ${worker.capabilities?.tradefed?'ok':''}">Tradefed ${worker.capabilities?.tradefed?'✓':'不可用'}</span></div>${(worker.warnings||[]).map(value=>`<div class="host-warning">⚠ ${esc(value)}</div>`).join('')}<div class="host-tests">${tests.map(test=>`<div><strong>${test.source==='external'?'手工/外部':'平台'} ${esc(test.suite_type||'XTS')}</strong> · PID ${esc(test.pid||'-')} · 设备 ${esc((test.devices||[]).join(', ')||'未识别')} · 运行 ${Math.floor((test.elapsed_seconds||0)/3600)}h</div>`).join('')||'<span class="muted">当前无测试</span>'}</div></div>`;
...
async function saveWorkerConfig(){
 ...
 try{await api(`/api/cluster/workers/${encodeURIComponent(configWorkerId)}/config`,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({max_jobs:maxJobs})});toast(`${configWorkerId} 配置已保存，Worker Agent 正在重启`);document.querySelector('#worker-config-modal').hidden=true;await refresh()}
...
async function autoDeployWorker(){
 ...
 const body={worker_id:normalizedWorkerId(document.querySelector('#new-worker-id').value),ssh_host:document.querySelector('#new-worker-host').value.trim(),controller_url:document.querySelector('#controller-url').value.trim(),suite_root:document.querySelector('#suite-root').value.trim(),token:document.querySelector('#worker-token').value,password:document.querySelector('#worker-password').value};
